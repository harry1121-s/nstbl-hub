...
contract EqLogicTest is BaseTest {
    ...
    function test_deposit() external {
        ...
        uint256 usdcBalBefore = IERC20Helper(USDC).balanceOf(address(eqlogic));
        uint256 usdtBalBefore = IERC20Helper(USDT).balanceOf(address(eqlogic));
        uint256 daiBalBefore = IERC20Helper(DAI).balanceOf(address(eqlogic));
        ...
        uint256 usdcBalAfter = IERC20Helper(USDC).balanceOf(address(eqlogic));
        uint256 usdtBalAfter = IERC20Helper(USDT).balanceOf(address(eqlogic));
        uint256 daiBalAfter = IERC20Helper(DAI).balanceOf(address(eqlogic));
        ...
        assertEq((usdcAmt + usdtAmt) * 1e12 + daiAmt, nstblToken.balanceOf(user1));
    }
    ...
    //     deal(USDT, user1, 2e6 * 1e6);

    //     // console.log(IERC20Helper(USDT).allowance(user1, address(this)));

    //     vm.startPrank(user1);
    //     IERC20Helper(USDT).safeIncreaseAllowance(address(this), 1e6 * 1e6);
    //     vm.stopPrank();

    //     // console.log(IERC20Helper(USDT).balanceOf(user1));
    //     // console.log(IERC20Helper(USDT).allowance(user1, address(this)));
    ...
    function test_validateAllocation_fuzz(
        ...
        (uint256 a1, uint256 a2, uint256 a3) = eqLogicHarness.validateSystemAllocation(_amt1, _amt2, _amt3);
        if (_p1 > dt && !flag) {
            if (_p2 > dt && _p3 > dt) {
                assertEq(a1, 8e3);
                assertEq(a2, 1e3);
